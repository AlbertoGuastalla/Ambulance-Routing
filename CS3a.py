...
def calculateWaitingTime(path, nodesInSolution, service_costs, redsCoordinates, numberOfReds,
    ...
    if secondNodeToExclude == path[currentNode]:
        timeUntilLastRed += (service_costs[currentNode - 1] + distanceMatrix[currentNode - 1][path[currentNode] - 1])
    else:
        timeUntilLastRed += (
...
def intraClusterSwapReds(tour, redsInSolution, redNodes, waitingTime, totalTime, service_costs,
    ...
    for swap in subsets:
        ...
        if tt <= tmax:
            newWaitingTime = calculateWaitingTime(p, nodesInSolution, service_costs, redNodes, len(redsInSolution),
                                                  distanceMatrix, startNode, endNode)
...
def deleteGreens(tours, greensInSolution, redsInSolution, hospitalNodes, redNodes,
    ...
    for i in range(0, len(tours)):
        ...
        tours[i] = newTour
        waitingTimes[i] = calculateWaitingTime(path, nodesInSolution, service_costs, redNodes,
                                               len(redsInSolution[i]), distanceMatrix, startNode,
                                               endNode)
...
def assignUniformHospitals(partitions, coordinates, m, partialDistanceMatrix, completeDistanceMatrix, redNodes):
    ...
    while flag:
        ...
        if flag:
            ...
            for c in range(0, len(clusters)):
                if len(clusters[c]) >= maxNodes and hospitalsInClusters[c] > minHospitalsInCluster[c]:
                    surplusClusters.append(c)
                if len(clusters[c]) < minNodes:
                    lackClusters.append(c)
...
partitions = assignUniformNodes(redsCoordinates, parameters[1], distanceMatrix_, distanceMatrix)
bestCombination = assignUniform(partitions, clusters, distanceMatrix)

for (p, c) in bestCombination:
    clusters[c] = {**clusters[c], **partitions[p]}
...
partitions = assignUniformHospitals(clusters, partialHospitalCoordinates, parameters[1], distanceMatrix_, distanceMatrix,
                                  redsCoordinates)
