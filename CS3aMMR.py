...
def interClusterMoveRed(tours, redNodes, waitingTimes, totalTimes, service_costs,
                        ...
                        greensInSolution, redsTabuTagIn, redsTabuTagOut, firstHopitalsMap, iteration, minimum_waiting_times):
                        ...
                        if h2 != firstHopitalsMap[t2]:
                            if (previous_t2[red2] in greensInSolution[t2] and p_t2[h2] != endNode) or (
                                ...
                                if mewTime_t1 <= tmax and newTime_t2 <= tmax:
                                    ...
                                    relative_maxmin_tour_t1 = -math.inf
                                    for r in newWaitingTime_t1:
                                        if relative_maxmin_tour_t1 < newWaitingTime_t1[r] / minimum_waiting_times[r]:
                                            relative_maxmin_tour_t1 = newWaitingTime_t1[r] / minimum_waiting_times[r]

                                    relative_maxmin_tour_t2 = -math.inf
                                    for r in newWaitingTime_t2:
                                        if relative_maxmin_tour_t2 < newWaitingTime_t2[r] / minimum_waiting_times[r]:
                                            relative_maxmin_tour_t2 = newWaitingTime_t2[r] / minimum_waiting_times[r]

                                    if relative_maxmin_tour_t1 + relative_maxmin_tour_t2 < min_:
                                        min_ = relative_maxmin_tour_t1 + relative_maxmin_tour_t2
...
def interClusterSwapReds(tours, redNodes, waitingTimes, totalTimes, service_costs, distanceMatrix,
                         tmax, startNode, endNode, redsInSolution, redsTabuTagIn, redsTabuTagOut, iteration, minimum_waiting_times):
    subsets = list(itertools.combinations(range(0, len(tours)), 2))
                ...
                if redsTabuTagOut[red1] < iteration and redsTabuTagIn[j][red1] < iteration:
                    if redsTabuTagOut[red2] < iteration and redsTabuTagIn[i][red2] < iteration:
                        ...
                        if mewTime_i <= tmax and newTime_j <= tmax:
                            ...
                            relative_maxmin_tour_i = -math.inf
                            for r in newWaitingTime_i:
                                if relative_maxmin_tour_i < newWaitingTime_i[r] / minimum_waiting_times[r]:
                                    relative_maxmin_tour_i = newWaitingTime_i[r] / minimum_waiting_times[r]

                            relative_maxmin_tour_j = -math.inf
                            for r in newWaitingTime_j:
                                if relative_maxmin_tour_j < newWaitingTime_j[r] / minimum_waiting_times[r]:
                                    relative_maxmin_tour_j = newWaitingTime_j[r] / minimum_waiting_times[r]

                            if relative_maxmin_tour_i + relative_maxmin_tour_j < min_:
                                min_ = relative_maxmin_tour_i + relative_maxmin_tour_j
...
def intraClusterSwapReds(tour, redsInSolution, redNodes, waitingTime, totalTime, service_costs,
                         distanceMatrix, tmax, startNode, endNode, minimum_waiting_times):
    path = {}
    previous = {}
    bestTour = None
    ...
    for swap in subsets:
        ...
        if tt <= tmax:
            ...
            relative_maxmin_tmp = -math.inf
            for r in tmp:
                if relative_maxmin_tmp < tmp[r] / minimum_waiting_times[r]:
                    relative_maxmin_tmp = tmp[r] / minimum_waiting_times[r]

            relative_maxmin_newWaitingTime = -math.inf
            for r in newWaitingTime:
                if relative_maxmin_newWaitingTime < newWaitingTime[r] / minimum_waiting_times[r]:
                    relative_maxmin_newWaitingTime = newWaitingTime[r] / minimum_waiting_times[r]

            if relative_maxmin_tmp > relative_maxmin_newWaitingTime:
                newTotalTime = tt
                tmp = newWaitingTime
                bestTour = p
...
def insertGreen(tours, redNodes, greenNodes, redsInSolution, greensInSolution, waitingTimes, totalTimes,
                ...
                greensTabuTagIn, iteration, objective, minimum_waiting_times):
    profit = 0
    tmp1 = math.inf
    bestTour = swap = None
                ...
                for n1, n2 in path.items():
                    ...
                    if n1 not in redsInSolution[t] and n2 != endNode and n1 != startNode:
                        ...
                        if newTotalTime <= tmax:
                            ...
                            if (profit < newProfit or (profit == newProfit and relative_maxmin_tmp > relative_maxmin_newWaitingTime) or
                                    (profit == newProfit and relative_maxmin_tmp == relative_maxmin_newWaitingTime and tmp1 > newTotalTime)) and flag:
                                greenInserted = green
                                tmp = newWaitingTime
                                tmp1 = newTotalTime
                                profit = newProfit
...
def swapGreens(tours, redNodes, greenNodes, redsInSolution, greensInSolution, waitingTimes, totalTimes,
               service_costs, distanceMatrix, tmax, scores, teamScores, startNode, endNode,
               greensTabuTagIn, greensTabuTagOut, iteration, objective, minimum_waiting_times):
    profit = 0
    tmp1 = math.inf
    bestTour = swap = None
    totalGreenInSolution = []
...
def localSearch(clusters, tours, service_costs, greenNodes, redNodes, hospitalNodes, nodeScores, waitingTimes,
                ...
                startNode, endNode, firstHopitalsMap, objective, minimum_waiting_times):
    bestTours = []
    bestRedsInSolution = {}
    ...
    while iteration < 100000:
        if CURRENT_OP == 0:
            ...
            for i in range(0, len(tours)):
                tours[i], waitingTimes[i], teamTotalTimes[i] = intraClusterSwapReds(tours[i], redsInSolution[i],
                                                                                    ...
                                                                                    startNode, endNode, minimum_waiting_times)
        ...
        if CURRENT_OP == 1:
            ...
            tours, waitingTimes, teamTotalTimes, swap = interClusterSwapReds(tours, redNodes, waitingTimes,
                                                                             ...
                                                                             redsTabuTagOut, iteration, minimum_waiting_times)
        ...
        if CURRENT_OP == 3:
            ...
            tours, waitingTimes, teamTotalTimes, teamScores, swap = swapGreens(tours, redNodes, greenNodes,
                                                                               ...
                                                                               greensTabuTagOut, iteration, objective, minimum_waiting_times)
        ...
        if CURRENT_OP == 4:
            ...
            tours, waitingTimes, teamTotalTimes, teamScores, swap = insertGreen(tours, redNodes, greenNodes,
                                                                                ...
                                                                                iteration, objective, minimum_waiting_times)
        ...
        if CURRENT_OP == 5:
            ...
            tours, waitingTimes, teamTotalTimes, swap = interClusterMoveRed(tours, redNodes, waitingTimes,
                                                                            ...
                                                                            firstHopitalsMap, iteration, minimum_waiting_times)
...
minimum_waiting_times = {}
for r in redsCoordinates:
    min_ = math.inf
    for start in firstHopitalsMap.values():
        for end in hospitalsCoordinates:
            completion_time = distanceMatrix[start - 1][r - 1] + distanceMatrix[r - 1][end - 1] + parameters[7][r - 1]

            if min_ > completion_time:
                min_ = completion_time

    minimum_waiting_times[r] = min_
...
localSearch(clusters, tours, parameters[7], greensCoordinates, redsCoordinates, hospitalsCoordinates, scores,
            ...
            teamScores, teamTotalTime, parameters[6][0], parameters[6][1], firstHopitalsMap, objective, minimum_waiting_times)
