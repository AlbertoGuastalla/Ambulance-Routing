                        ...
                        if h2 != firstHopitalsMap[t2]:
                            if (previous_t2[red2] in greensInSolution[t2] and p_t2[h2] != endNode) or (
                                ...
                                if mewTime_t1 <= tmax and newTime_t2 <= tmax:
                                    ...
                                    sum_pf = 0
                                    for r in newWaitingTime_t1.keys():
                                        if r == red2:
                                            sum_pf += newWaitingTime_t1[r] - waitingTimes[t2][r]
                                        else:
                                            sum_pf += newWaitingTime_t1[r] - waitingTimes[t1][r]

                                    for r in newWaitingTime_t2.keys():
                                        if r == red2:
                                            sum_pf += newWaitingTime_t2[r] - waitingTimes[t1][r]
                                        else:
                                            sum_pf += newWaitingTime_t2[r] - waitingTimes[t2][r]
                ...
                if redsTabuTagOut[red1] < iteration and redsTabuTagIn[j][red1] < iteration:
                    if redsTabuTagOut[red2] < iteration and redsTabuTagIn[i][red2] < iteration:
                        ...
                        if mewTime_i <= tmax and newTime_j <= tmax:
                            ...
                            sum_pf = 0
                            for r in newWaitingTime_i.keys():
                                if r == red2:
                                    sum_pf += newWaitingTime_i[r] - waitingTimes[j][r]
                                else:
                                    sum_pf += newWaitingTime_i[r] - waitingTimes[i][r]

                            for r in newWaitingTime_j.keys():
                                if r == red1:
                                    sum_pf += newWaitingTime_j[r] - waitingTimes[i][r]
                                else:
                                    sum_pf += newWaitingTime_j[r] - waitingTimes[j][r]
...
def intraClusterSwapReds(tour, redsInSolution, redNodes, waitingTime, totalTime, service_costs,
    ...
    for swap in subsets:
        ...
        if tt <= tmax:
            ...
            sum_pf = 0
            for r in newWaitingTime.keys():
                sum_pf += newWaitingTime[r] - waitingTime[r]
                ...
                for n1, n2 in path.items():
                    ...
                    if n1 not in redsInSolution[t] and n2 != endNode and n1 != startNode:
                        ...
                        if newTotalTime <= tmax:
                            ...
                            sum_pf = 0
                            for r in newWaitingTime.keys():
                                sum_pf += newWaitingTime[r] - waitingTimes[t][r]

                            if (profit < newProfit or (profit == newProfit and tmp > sum_pf) or
                                    (profit == newProfit and tmp == sum_pf and tmp1 > newTotalTime)) and flag:
                                greenInserted = green
                                tmp = sum_pf
                                tmp1 = newTotalTime
                                profit = newProfit
